...
namespace One
{
    class Solution
    {
        ...
        static int Search(List<int> lst, int n)
        {
            int low = 0;
            int high = lst.Count;
            while (low <= high)
        ...
        static void Main()
        {
            ...
            int elementsN = int.Parse(Console.ReadLine());
            ...
            for (int i = 0; i < elementsN; i++)
            {
                int result = Search(lst, number);
                if (result== 0)
                {
                    Console.WriteLine("The element was not found int the numbers");
                }
                else
                {
